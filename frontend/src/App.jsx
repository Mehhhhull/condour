...
import { useRedditScan } from './hooks/useRedditScan';

export default function App() {
    const { status, logs, results, startScan, reset } = useRedditScan();
                    ...
                    {status === 'complete' && results && (
                        <div className="w-full">
                            <div className="flex justify-between items-center border-b border-green-900/50 pb-6 mb-8">
                                <div>
                                    <h1 className="font-serif text-2xl text-white">Analysis Report: <span className="text-zinc-400">{results.productName}</span></h1>
                                    <p className="font-mono text-xs text-green-500 mt-1">LIVE REDDIT DATA • {results?.stats?.totalPosts || 0} POSTS • {results?.stats?.totalComments || 0} COMMENTS</p>
                                </div>
                                <div className="flex gap-4">
                                    <button onClick={reset} className="text-zinc-500 hover:text-white text-xs uppercase tracking-widest transition-colors cursor-none">
                                        New Scan
